...
```bash
sales-copilot schedule_brief --kind morning     # 준비 안 됐으면 종료코드 2 로 거부하고 무엇이 빠졌는지 알린다
```
...
- **가드레일(fail-closed)**: 같은 자동 발송 액션은 24시간 쿨다운·하루 실행 횟수 상한(기본 5회), 오늘 발송량·한도를 확인하지 못하면 발송하지 말고 승인 대기로 내린다. 이상 신호(스팸 신고·회신 항의 급증·반송률 급등) 감지 시 해당 시퀀스 자동 정지 후 보고.
