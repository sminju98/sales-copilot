...
```
- 로컬에서 회사소개서·제품자료·가격표·제안서·고객사례·FAQ를 찾아 읽는다(CTX-01). 드라이브·노션 등 커넥터가 연결돼 있으면 그쪽 자료를 우선하고, 미연결이면 로컬만으로 동작한다.
...
- 대화 중 가격 개편·신제품·조직 변경이 감지되면 해당 파일 갱신을 즉시 제안한다.
...
질문 {N}개: 1) {…} 2) {…}
다음: [[icp]]로 이번 주 세그먼트 확정 → 오늘 큐에 신규 접촉 등록
```
