...
## 실행 모드 — 준비된 명단에 바로도, 발굴부터 풀시퀀스도 ([[method]] §4-1)
- **단독·명단 있음("이 명단에 콜드메일만 / 이 리드한테 메일 써줘")**: 3단계(발굴)를 건너뛰고 4단계(리서치)는 가벼운 패스만 → 5 작성 → 6 적재 → 발송 게이트. **이 캠페인만 돌리고 전화·소개로 자동 확장하지 않는다** — 후속 시퀀스는 이 명단 안에서만 돈다.
- **풀시퀀스("발굴부터 / 알아서 다 돌려 / 리드 찾아서")** 또는 [[find-leads]]·[[today]]에서 넘어옴: 아래 1~7단계 전체.
- **체인 아웃("무응답이면 전화까지")**: 10일차에서 [[phone-call]]로 넘긴다. 단독이면 여기까지고, 전화 확장은 명시할 때만.
- 어느 모드든 **철칙 4개·발송 게이트는 생략 불가**(아래).
