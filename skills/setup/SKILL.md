...
## 0. 준비 — 상태 진단
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/doctor.py"        # config·context 8종·CRM 파일·수신거부 목록 점검
python3 "$CLAUDE_PLUGIN_ROOT/scripts/quicksetup.py"    # ⚡ 빠른 길: 반자동 설정(config 골격·context/ 템플릿 준비)
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.name="홍길동" me.rank="rep" me.title="영업팀 대리 (AE)" me.functions="outbound,followup"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.sell_what="중소 이커머스에 AI 광고 소재 구독" company.name="우리 회사" company.one_liner="무엇을, 누구에게, 왜 좋은지 한 줄"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.accounts_scope="assigned" me.info_scope="own"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.send_scope="find_leads,register_contacts,draft_email,propose_meeting" me.approval_mode="per_item"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.send_as="self" me.sender_email="gildong@company.com" me.signature="홍길동 드림 | OO 영업팀"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" me.escalate_rules="예상 계약금액 3천만원 이상,할인율 10% 초과,계약조건 변경" me.personal_contacts_policy="selected"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/set_config.py" sources.use_email=true sources.use_calendar=true sources.use_crm=false sources.cards_inbox="~/Pictures/명함"
```
